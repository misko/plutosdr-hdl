...
if {$argc ni {3 4}} { error "expected NEW_OUTPUT FROZEN_VECTORS FAST_MHZ ?join-gate-removed?" }
...
set mutation [expr {$argc == 4}]
if {$mutation && [lindex $argv 3] ne "join-gate-removed"} { error "unsupported mutation" }
...
set_property generic [list FAST_MHZ=$fast_mhz QUICK_MUTATION=$mutation] [get_filesets sim_1]
