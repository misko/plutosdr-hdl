...
output_dir="${1:-${script_dir}/build/spectrum-product-ooc}"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$script_dir/synthesize_spectrum_product_ooc.tcl" \
      -tclargs "$output_dir" 2>&1 | tee spectrum_product_ooc_transcript.log
)
