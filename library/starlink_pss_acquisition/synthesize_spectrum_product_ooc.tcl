# Vivado 2022.2 OOC gate for the exact Q1.23 complex spectrum product.
# Usage: vivado -mode batch -source synthesize_spectrum_product_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_spectrum_product.v]
read_xdc [file join $script_dir starlink_pss_spectrum_product_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_spectrum_product \
...
foreach {name contents} [list \
    starlink_pss_spectrum_product_utilization_opt.rpt $utilization_report \
    starlink_pss_spectrum_product_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_spectrum_product_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_spectrum_product_check_timing.rpt] w]
...
if {[llength $ramb36_cells] != 0 || [llength $ramb18_cells] != 0} {
  error "product must infer no BRAM, got RAMB36/RAMB18 [llength $ramb36_cells]/[llength $ramb18_cells]"
}
if {[llength $dsp_cells] != 8} {
  error "expected eight DSP48E1 cells, got [llength $dsp_cells]"
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_spectrum_product +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_spectrum_product +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 1200 || $total_ffs > 1000} {
  error "product logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_spectrum_product_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_spectrum_product_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "component_width=24"
puts $summary "fraction_bits=23"
puts $summary "product_safety_shift=1"
puts $summary "rounding=nearest_ties_even"
...
puts "STARLINK_SPECTRUM_PRODUCT_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs dsp48e1=[llength $dsp_cells]"
close_design
