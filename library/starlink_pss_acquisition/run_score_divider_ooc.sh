...
output_dir="${1:-${script_dir}/build/score-divider-ooc}"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$script_dir/synthesize_score_divider_ooc.tcl" \
      -tclargs "$output_dir" 2>&1 | tee score_divider_ooc_transcript.log
)
