# Experimental 255-tap pilot FIR standalone resource/timing investigation.
...
if {$argc != 1} { error "expected an absolute output directory" }
if {[version -short] ne "2022.2"} { error "requires Vivado 2022.2" }
set script_dir [file dirname [file normalize [info script]]]
set output_dir [file normalize [lindex $argv 0]]
file mkdir $output_dir
cd $script_dir
read_verilog [file join $script_dir starlink_pilot_fir3.v]
...
synth_design -mode out_of_context -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high -top starlink_pilot_fir3 -part xc7z010clg400-1
opt_design -directive ExploreArea
place_design -directive Explore
phys_opt_design -directive Explore
route_design -directive Explore

write_checkpoint -force [file join $output_dir pilot_fir3_routed.dcp]
...
foreach source_file {starlink_pilot_fir3.v pilot_fir3_q17.mem starlink_pss_x2_ddc_ooc.xdc} {
  puts $channel "source_sha256=[exec sha256sum [file join $script_dir $source_file]]"
}
close $channel
if {$dsps != 8 || $bram36 + 0.5 * $bram18 > 4 || $luts > 1800 || $registers > 2000} {
...
close $channel
puts "STARLINK_PILOT_FIR3_INTERNAL_OOC_PASS LUT=$luts FF=$registers DSP=$dsps BRAM36=$bram36 BRAM18=$bram18 WNS=$wns WHS=$whs IO_AND_FULL_RECEIVER_UNQUALIFIED"
close_design
