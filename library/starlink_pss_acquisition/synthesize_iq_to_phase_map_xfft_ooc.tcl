# Vivado 2022.2 OOC gate for the complete 15 MS/s CI16-to-phase-map path.
# Usage: vivado -mode batch -source synthesize_iq_to_phase_map_xfft_ooc.tcl \
#        -tclargs OUTPUT
...
set project_name starlink_pss_iq_to_phase_map_xfft_ooc
...
add_files -fileset constrs_1 -norecurse \
  [file join $script_dir starlink_pss_iq_to_phase_map_xfft_ooc.xdc]
set_property top starlink_pss_iq_to_phase_map [get_filesets sources_1]
...
if {[get_property STATUS [get_runs synth_1]] ne "synth_design Complete!"} {
  error "phase-map composition synthesis did not complete"
}
open_run synth_1
...
foreach {name contents} [list \
    starlink_pss_iq_to_phase_map_xfft_utilization_opt.rpt $utilization_report \
    starlink_pss_iq_to_phase_map_xfft_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_iq_to_phase_map_xfft_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_iq_to_phase_map_xfft_check_timing.rpt] w]
...
if {![regexp {\| starlink_pss_iq_to_phase_map +\| +\(top\) +\| +([0-9]+) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|} \
...
write_checkpoint -force \
  [file join $output_dir starlink_pss_iq_to_phase_map_xfft_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_iq_to_phase_map_xfft_ooc_summary.txt] w]
...
puts $summary "phase_bins=20000"
puts $summary "tile_frames=64"
puts $summary "bounded_map_bytes=40000"
...
puts "STARLINK_IQ_TO_PHASE_MAP_XFFT_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$lut_count total_ffs=$register_count ramb36e1=$ramb36_count ramb18e1=$ramb18_count dsp48e1=$dsp_count"
close_design
close_project
