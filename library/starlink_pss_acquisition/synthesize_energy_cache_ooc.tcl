# Vivado 2022.2 OOC gate for the exact CI16 sliding-energy cache.
# Usage: vivado -mode batch -source synthesize_energy_cache_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_energy_cache.v]
read_xdc [file join $script_dir starlink_pss_energy_cache_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_energy_cache \
...
foreach {name contents} [list \
    starlink_pss_energy_cache_utilization_opt.rpt $utilization_report \
    starlink_pss_energy_cache_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_energy_cache_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_energy_cache_check_timing.rpt] w]
...
if {[llength $ramb36_cells] != 2 || [llength $ramb18_cells] != 1} {
  error "expected RAMB36/RAMB18 counts 2/1, got [llength $ramb36_cells]/[llength $ramb18_cells]"
}
if {[llength $dsp_cells] != 2} {
  error "expected two DSP48E1 cells, got [llength $dsp_cells]"
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_energy_cache +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_energy_cache +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 1000 || $total_ffs > 800} {
  error "energy-cache logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_energy_cache_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_energy_cache_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "window_samples=66"
puts $summary "cache_entries=2048"
puts $summary "energy_bits=38"
...
puts "STARLINK_ENERGY_CACHE_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs ramb36e1=[llength $ramb36_cells] dsp48e1=[llength $dsp_cells]"
close_design
