# Vivado 2022.2 OOC gate for the hash-locked 512-bin PSS kernel ROM.
# Usage: vivado -mode batch -source synthesize_kernel_rom_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_kernel_rom.v]
read_xdc [file join $script_dir starlink_pss_kernel_rom_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -generic [list ROM_FILE=$memory_file] \
  -top starlink_pss_kernel_rom \
...
foreach {name contents} [list \
    starlink_pss_kernel_rom_utilization_opt.rpt $utilization_report \
    starlink_pss_kernel_rom_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_kernel_rom_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_kernel_rom_check_timing.rpt] w]
...
if {[llength $ramb36_cells] != 0 || [llength $ramb18_cells] != 2 ||
    [llength $dsp_cells] != 0} {
  error "kernel must use exactly two RAMB18 and no RAMB36/DSP, got [llength $ramb36_cells]/[llength $ramb18_cells]/[llength $dsp_cells]"
}

set nonzero_init_words 0
foreach memory_cell $ramb18_cells {
  set cell_nonzero_init_words 0
  foreach property_name [list_property $memory_cell] {
    if {[regexp {^INIT_[0-9A-F][0-9A-F]$} $property_name] &&
        [get_property $property_name $memory_cell] ne [string repeat 0 64]} {
      incr cell_nonzero_init_words
      incr nonzero_init_words
    }
  }
  if {$cell_nonzero_init_words == 0} {
    error "inferred RAMB18 has no nonzero INIT payload: $memory_cell"
  }
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_kernel_rom +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_kernel_rom +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 500 || $total_ffs > 500} {
  error "kernel-ROM logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force [file join $output_dir starlink_pss_kernel_rom_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_kernel_rom_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "kernel_bins=512"
puts $summary "kernel_format=signed_q1_23"
puts $summary "memory_word_format=q_then_i_48_bits"
puts $summary "canonical_binary_sha256=d96c56b3d6bcd03419a57f23f3ce4929f1e478663119f5cb5ec9b14327b7ff2b"
puts $summary "memory_text_sha256=7c89ff2a026f5fab91e655ab969ac07c11bf9715215173dadec07084527aea7d"
...
puts $summary "bram_tiles=1.0"
puts $summary "nonzero_init_words=$nonzero_init_words"
puts $summary "dsp48e1=[llength $dsp_cells]"
close $summary

puts "STARLINK_KERNEL_ROM_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs ramb18e1=[llength $ramb18_cells] nonzero_init_words=$nonzero_init_words"
close_design
