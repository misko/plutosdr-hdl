# Vivado 2022.2 OOC gate for one strict generated-XFFT boundary adapter.
# Usage: vivado -mode batch -source synthesize_xfft_block_adapter_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_xfft_block_adapter.v]
read_xdc [file join $script_dir starlink_pss_xfft_block_adapter_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_xfft_block_adapter \
...
foreach {name contents} [list \
    starlink_pss_xfft_block_adapter_utilization_opt.rpt $utilization_report \
    starlink_pss_xfft_block_adapter_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_xfft_block_adapter_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_xfft_block_adapter_check_timing.rpt] w]
...
if {[llength $ramb36_cells] != 0 || [llength $ramb18_cells] != 0 ||
    [llength $dsp_cells] != 0} {
  error "adapter must use no RAMB36/RAMB18/DSP, got [llength $ramb36_cells]/[llength $ramb18_cells]/[llength $dsp_cells]"
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_xfft_block_adapter +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_xfft_block_adapter +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 600 || $total_ffs > 600} {
  error "XFFT adapter logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_xfft_block_adapter_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_xfft_block_adapter_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "transform_samples=512"
puts $summary "transform_direction=forward"
puts $summary "data_bits=24"
puts $summary "tuser_bits=24"
puts $summary "status_bits=8"
puts $summary "maximum_blocks_inflight=1"
puts $summary "core_reset_low_cycles_after_flush=2"
...
puts "STARLINK_XFFT_BLOCK_ADAPTER_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs"
close_design
