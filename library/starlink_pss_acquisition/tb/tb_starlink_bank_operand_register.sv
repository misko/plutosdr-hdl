`timescale 1ns/1ps
module tb_starlink_bank_operand_register;
  parameter integer D=18, B=0, P=0;
  ...
  generate for(genvar r=0;r<2;r=r+1) begin: mode
    starlink_pss_spectrum_product_operand_register #(
      .DATA_WIDTH(D),.REGISTER_OPERANDS(r),.BOUNDARY_ROUND_SAT(B),.PRIVATE_PAYLOAD_BUBBLES(P)) dut (
  ...
  starlink_pss_spectrum_product #(.DATA_WIDTH(D),.PRIVATE_PAYLOAD_BUBBLES(P)) golden (
  ...
  reg [1023:0] path;
  reg [4*D+79-1:0] expected_operand_payload;
  integer private_bubble_captures=0;
  ...
  initial begin
    ...
    for(cycle=0;cycle<5010;cycle=cycle+1) begin
      ...
      #1;
      expected_operand_payload=mode[1].dut.registered_operands.payload;
      if(!resetn || flush) expected_operand_payload=0;
      else if(mode[1].dut.registered_operands.ready) begin
        if(P || iv[1]) expected_operand_payload[4*D+79-1:79]={ii[1],iq[1],ki[1],kq[1]};
        if(iv[1]) expected_operand_payload[78:0]={bin[1],exponent[1],last[1],start[1]};
        else if(P) private_bubble_captures=private_bubble_captures+1;
      end
      for(r=0;r<2;r=r+1) begin
        depth=3+r;
      ...
      #0.001;
      if(mode[1].dut.registered_operands.payload !== expected_operand_payload)
        $fatal(1,"OPERAND_PRIVATE_BUBBLE_OWNERSHIP_MISMATCH cycle=%0d",cycle);
    ...
    for(r=0;r<2;r=r+1) begin
      ...
      $display("OPERAND_MODE_PASS width=%0d round=%0d private_bubbles=%0d registered=%0d no_stall_elapsed_clocks=%0d accepted=%0d emitted=%0d discarded=%0d simultaneous=%0d stalls=%0d pulses=%0d last_saturation_reset=1 last_saturation_flush=1 outstanding=0",
        D,B,P,r,2+r,accepted[r],emitted[r],discarded[r],simultaneous[r],stalls[r],pulses[r]);
    end
    if(P && private_bubble_captures==0) $fatal(1,"OPERAND_PRIVATE_BUBBLE_COVERAGE_MISSING");
    $display("OPERAND_PRIVATE_BUBBLES_PASS enabled=%0d captures=%0d metadata_token_occupied_preserved=1",P,private_bubble_captures);
