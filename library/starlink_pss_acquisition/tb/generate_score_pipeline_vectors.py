#!/usr/bin/env python3
"""Generate independent exact vectors for raw-correlation to 8-bit PSS score."""
...
def exact_score(
    ...
    shift = 2 * (1 + forward_exponent + inverse_exponent)
    mathematical_numerator = power << shift
    numerator_saturated = mathematical_numerator > RATIO_MAX
    numerator = RATIO_MAX if numerator_saturated else mathematical_numerator
    denominator = sample_energy * COEFFICIENT_ENERGY
    denominator_zero = denominator == 0
    if numerator == 0 or denominator_zero:
        return 0, numerator_saturated, denominator_zero
    if numerator >= denominator:
        return 255, numerator_saturated, denominator_zero
    ...
    if doubled_remainder > denominator or (
        ...
        quotient += 1
    return quotient, numerator_saturated, denominator_zero
...
def write_vectors(path: Path, random_count: int) -> None:
    rng = random.Random(0x50535370697065)
