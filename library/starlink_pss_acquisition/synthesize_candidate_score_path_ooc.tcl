# Vivado 2022.2 OOC gate for the composed IFFT-to-score candidate path.
# Usage: vivado -mode batch -source synthesize_candidate_score_path_ooc.tcl \
#        -tclargs OUTPUT
...
foreach source_name {
  ...
  read_verilog [file join $script_dir $source_name]
}
read_xdc [file join $script_dir starlink_pss_candidate_score_path_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_candidate_score_path \
...
foreach {name contents} [list \
    starlink_pss_candidate_score_path_utilization_opt.rpt $utilization_report \
    starlink_pss_candidate_score_path_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_candidate_score_path_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_candidate_score_path_check_timing.rpt] w]
...
foreach {label pattern} {
  total_luts {\| starlink_pss_candidate_score_path +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_candidate_score_path +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 3000 || $total_ffs > 2500} {
  error "candidate-score path logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_candidate_score_path_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_candidate_score_path_ooc_summary.txt] w]
...
puts "STARLINK_CANDIDATE_SCORE_PATH_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs ramb36e1=[llength $ramb36_cells] dsp48e1=[llength $dsp_cells]"
close_design
