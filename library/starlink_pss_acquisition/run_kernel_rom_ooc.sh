...
output_dir="${1:-${script_dir}/build/kernel-rom-ooc}"
...
python3 "$script_dir/tb/verify_upper_edge_pss_kernel.py"
mkdir -p "$output_dir"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$script_dir/synthesize_kernel_rom_ooc.tcl" \
      -tclargs "$output_dir" 2>&1 | tee kernel_rom_ooc_transcript.log
)
