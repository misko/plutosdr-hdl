// Elastic Q1.23 complex spectrum product for Starlink PSS acquisition.
//
// The forward-XFFT and precomputed-kernel components are both signed Q1.23.
// Their complex product is divided by 2**24: 23 fractional bits plus the
// frozen one-bit safety shift. Rounding is signed round-to-nearest,
...
module starlink_pss_spectrum_product (
  ...
  input  wire signed [23:0]   kernel_i,
  input  wire signed [23:0]   kernel_q,
  ...
  output reg signed [23:0]    output_i,
  output reg signed [23:0]    output_q,
  ...
  localparam integer ROUND_SHIFT = 24;
  localparam signed [49:0] COMPONENT_MAX = 50'sd8388607;
  localparam signed [49:0] COMPONENT_MIN = -50'sd8388608;

  (* use_dsp = "yes" *) reg signed [47:0] product_ii;
  (* use_dsp = "yes" *) reg signed [47:0] product_qq;
  (* use_dsp = "yes" *) reg signed [47:0] product_iq;
  (* use_dsp = "yes" *) reg signed [47:0] product_qi;
  reg product_valid;
  ...
  reg signed [48:0] sum_real;
  reg signed [48:0] sum_imag;
  reg sum_valid;
  ...
  wire [24:0] rounded_real;
  wire [24:0] rounded_imag;
  ...
  // Bit 24 is the overflow flag; bits 23:0 are the saturated Q1.23 result.
  function automatic [24:0] round_and_saturate;
    input signed [48:0] value;
    reg signed [48:0] quotient;
    reg [ROUND_SHIFT-1:0] remainder;
    reg increment;
    reg signed [49:0] rounded;
    begin
      quotient = value >>> ROUND_SHIFT;
      remainder = value[ROUND_SHIFT-1:0];
      increment = (remainder > (1 << (ROUND_SHIFT - 1))) ||
        ((remainder == (1 << (ROUND_SHIFT - 1))) && quotient[0]);
      rounded = $signed({quotient[48], quotient}) + increment;
      if (rounded > COMPONENT_MAX)
        round_and_saturate = {1'b1, 24'h7fffff};
      else if (rounded < COMPONENT_MIN)
        round_and_saturate = {1'b1, 24'h800000};
      else
        round_and_saturate = {1'b0, rounded[23:0]};
    end
  endfunction
  ...
  always @(posedge clk) begin
    if (!resetn || flush) begin
      ...
      if (output_stage_ready) begin
        ...
        if (sum_valid) begin
          output_i <= rounded_real[23:0];
          output_q <= rounded_imag[23:0];
          ...
          output_overflow <= rounded_real[24] || rounded_imag[24];
          overflow_pulse <= rounded_real[24] || rounded_imag[24];
        end else begin
          output_overflow <= 1'b0;
        end
      end
      ...
      if (sum_stage_ready) begin
        ...
        if (product_valid) begin
          sum_real <= $signed({product_ii[47], product_ii}) -
                      $signed({product_qq[47], product_qq});
          sum_imag <= $signed({product_iq[47], product_iq}) +
                      $signed({product_qi[47], product_qi});
