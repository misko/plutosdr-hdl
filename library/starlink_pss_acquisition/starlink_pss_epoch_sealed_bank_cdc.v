...
module starlink_pss_epoch_sealed_bank_cdc #(
  ...
  generate if (!SEALED_PUBLICATION) begin : legacy
    starlink_pss_block_mailbox #(.METADATA_WIDTH(75), .EXPLICIT_COMMIT(1), .RESET_RELEASE_EXTERNAL(1)) original (
      ...
      .output_clk(output_clk), .output_resetn(output_resetn), .output_valid(output_valid),
    ...
    always @(posedge input_clk or negedge input_resetn) begin
      if (!input_resetn) begin
        ...
        // CDC reader state lives below fast control.
    ...
    // Actual slow-domain read/ACK state. No raw fast fault/publication flag
