# Vivado 2022.2 OOC gate for the 512-entry raw IFFT-result FIFO.
# Usage: vivado -mode batch -source synthesize_raw_result_fifo_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_raw_result_fifo.v]
read_xdc [file join $script_dir starlink_pss_raw_result_fifo_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_raw_result_fifo \
...
foreach {name contents} [list \
    starlink_pss_raw_result_fifo_utilization_opt.rpt $utilization_report \
    starlink_pss_raw_result_fifo_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_raw_result_fifo_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_raw_result_fifo_check_timing.rpt] w]
...
if {[llength $dsp_cells] != 0} {
  error "raw-result FIFO must not consume DSP48E1 cells"
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_raw_result_fifo +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_raw_result_fifo +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 500 || $total_ffs > 400} {
  error "raw-result FIFO logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_raw_result_fifo_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_raw_result_fifo_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "fifo_depth=512"
puts $summary "payload_bits=123"
puts $summary "qualified_ifft_burst_results=447"
...
puts "STARLINK_RAW_RESULT_FIFO_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs ramb36e1=[llength $ramb36_cells]"
close_design
