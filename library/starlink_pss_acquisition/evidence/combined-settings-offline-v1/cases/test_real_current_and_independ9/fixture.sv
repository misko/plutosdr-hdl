...
module status_guard_probe #(parameter integer GOLDEN=0, MODE=0)(
 ...
 generate if(GOLDEN)begin:g_old
  ...
  assign phase_tuple={dut.active,dut.awaiting_ack,dut.return_last,
   dut.final_qualified,dut.exponent_seen,dut.output_exponent};
...
module tb_starlink_pss_fft_bank_owned_slice;
 ...
 integer phase_witnesses=0,coincident_output_rows=0,ready_ack_rows=0;
 ...
 endtask
 task witness_phase(input integer phase_number);
  reg[9:0]expected_phase;
  #0.02;
  case(phase_number)
   0:expected_phase={1'b0,1'b0,1'b0,1'b0,1'b0,5'd0};
   1:expected_phase={1'b1,1'b0,1'b0,1'b0,1'b0,5'd0};
   2:expected_phase={1'b1,1'b0,1'b0,1'b0,1'b1,5'd5};
   3:expected_phase={1'b1,1'b0,1'b1,1'b0,1'b1,5'd5};
   4:expected_phase={1'b1,1'b0,1'b1,1'b1,1'b1,5'd5};
   5:expected_phase={1'b0,1'b1,1'b1,1'b1,1'b1,5'd5};
  endcase
  if(c_phase!==expected_phase || r_phase!==expected_phase)
   $fatal(1,"STATUS_GUARD_PHASE_NOT_REACHED phase=%0d actual=%b reference=%b expected=%b",
    phase_number,c_phase,r_phase,expected_phase);
  phase_witnesses=phase_witnesses+1;
 endtask
 ...
 initial begin
  ...
  // Separate additive epochs: a status on the SAME arriving output beat.
  // The prior72 rows deliberately injected status without an output beat.
  phase_id=2;
  for(kind=12;kind<15;kind=kind+1)begin
   prepare_phase(phase_id);witness_phase(phase_id);
   core_output_tvalid=1;core_output_tlast=0;
   core_output_tuser={3'b0,5'd5,7'b0,9'd1};core_status_tvalid=1;
   case(kind)
    12:begin candidate_status=5;expected_reasons=0;end
    13:begin candidate_status=4;expected_reasons=8'h30;end
    14:begin candidate_status=8'h25;expected_reasons=8'h10;end
   endcase
   reference_status=candidate_status;#0.02;
   if(!core_output_tvalid || !core_status_tvalid || c_phase[9]!==1 || c_phase[5:0]!=={1'b1,5'd5})
    $fatal(1,"STATUS_OUTPUT_COINCIDENCE_NOT_REACHED");
   if(c_valid!==(kind==12) || r_valid!==(kind==12))
    $fatal(1,"STATUS_OUTPUT_CURRENT_VETO_WRONG");
   tick();
   if(c_reasons!==expected_reasons || r_reasons!==expected_reasons)
    $fatal(1,"STATUS_OUTPUT_STICKY_REASONS_WRONG");
   @(negedge clk);quiet();repeat(2)tick();coincident_output_rows=coincident_output_rows+1;
  end
  // Actual ready-high ACK attempt: idle status/orphan events must retain
  // ownership on this edge, latch exact reasons, and prevent new admission.
  phase_id=5;
  for(kind=15;kind<17;kind=kind+1)begin
   prepare_phase(phase_id);witness_phase(phase_id);
   mailbox_input_ready=1;core_status_tvalid=1;
   candidate_status=5;reference_status=5;core_output_tvalid=(kind==16);
   expected_reasons=(kind==16)?8'h30:8'h10;#0.02;
   if(c_phase[9:8]!==2'b01 || !mailbox_input_ready || !core_status_tvalid)
    $fatal(1,"STATUS_READY_ACK_ATTEMPT_NOT_REACHED");
   if(c_valid===1 || c_commit===1 || r_valid===1 || r_commit===1)
    $fatal(1,"STATUS_READY_ACK_PUBLIC_ESCAPE");
   tick();
   if(c_phase[8]!==1 || r_phase[8]!==1 || c_reasons!==expected_reasons || r_reasons!==expected_reasons)
    $fatal(1,"STATUS_READY_ACK_OWNERSHIP_OR_REASON_LOST");
   @(negedge clk);quiet();repeat(2)tick();
   if(c_phase[8]!==1 || r_phase[8]!==1)$fatal(1,"STATUS_READY_ACK_STICKY_OWNERSHIP_LOST");
   ready_ack_rows=ready_ack_rows+1;
  end
  $display("STATUS_REAL_GUARD_PASS mode=%0d rows=%0d phases=6 invalid_payload_rows=%0d known_reserved_reasons=%0d checks=%0d phase_witnesses=%0d coincident_output_rows=%0d ready_ack_rows=%0d synthetic_core_not_fft=1",
   MODE,rows,invalid_rows,known_reasons,checks,phase_witnesses,coincident_output_rows,ready_ack_rows);$finish;
 end
endmodule
