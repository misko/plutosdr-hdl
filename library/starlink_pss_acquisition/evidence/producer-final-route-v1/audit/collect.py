...
RUN = ROOT / 'product-final-route-v1'
...
assert identity(PREP / 'SHA256SUMS')['sha256'] == 'b3c446578c0c35f8e0f199aa12f66d8635b6f7d21c9b97713a5ffed2ac2b0f06'
prepared = {}
...
scope = (ROOT / 'product-final-synthesis-v1/synthesis/scope.txt').read_text()
...
write('independent-post-audit.json', {
    'original_handle': 63593, 'owner_terminal_exit': 0,
    'original_terminal': terminal, 'required_nonempty_products': 15,
    'all_products_rehashed': True, 'prepared_19_hashes': prepared,
    'synthesis_source_ip_and_actual_log_rows_rehashed': 15,
...
copy(ROOT / 'product-final-synthesis-v1/synthesis/fft_bank_owned_synth.dcp', Path('input/fft_bank_owned_synth.dcp'))
copy(PREP / 'route_completed_input_fence.tcl', Path('input/route_completed_input_fence.tcl'))
copy(ROOT / 'product-final-synthesis-v1/synthesis/scope.txt', Path('input/synthesis_scope.txt'))
copy(ROOT / 'product-final-synthesis-v1/terminal.json', Path('input/synthesis_terminal.json'))
copy(PREP / 'SHA256SUMS', Path('input/prepared-SHA256SUMS'))
baseline = ROOT / 'rom-k1m1-route-v1/route'
for name in ('timing_unqualified.rpt', 'utilization.rpt', 'receipt.txt', 'island_175_max.rpt', 'cdc_unqualified.rpt'):
    copy(baseline / name, Path('baseline-ROM-route') / name)
