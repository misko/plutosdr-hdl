...
source settings.tcl
cd $old_directory
if {[llength $exact_generics] != 9} { error "unexpected option inventory" }
foreach key {REGISTERED_SCHEDULING DISTRIBUTED_FAST_FAULT PRIVATE_NEXT_START_SCRATCH EXACT_EXTRA_EPOCHS PER_CAUSE_FAULT_CDC PRIVATE_ROM_READ_AHEAD PRIVATE_BLOCK_METADATA_READ_AHEAD} {
...
puts [exec env -u PYTHONHOME -u PYTHONPATH -u LD_LIBRARY_PATH /home/mouse9911/gits/pluto-plus-utils/.venv/bin/python -B [file join $source_dir prepare_rom_read_ahead_actual.py] --verify-prepared $output_dir]
