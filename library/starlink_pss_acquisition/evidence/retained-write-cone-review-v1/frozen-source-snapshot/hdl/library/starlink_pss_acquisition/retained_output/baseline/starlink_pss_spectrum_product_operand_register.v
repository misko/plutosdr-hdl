// Additive bank-arithmetic study. Original bank/core bindings are unchanged.
  ...
  generate if (REGISTER_OPERANDS == 1) begin : registered_operands
    ...
    always @(posedge clk) begin
      if (!resetn || flush) begin
        ...
        valid <= input_valid;
        // Unowned numerical bubble computation never creates metadata/tokens.
        // Occupied stalled payload remains protected by the outer ready gate.
        if (PRIVATE_PAYLOAD_BUBBLES || input_valid)
          payload[4*DATA_WIDTH+79-1:79] <= {input_i, input_q, kernel_i, kernel_q};
        if (input_valid)
          payload[78:0] <= {input_bin_index, input_block_exponent, input_last,
