...
Status: design review only. No RTL, tested helper, observer, constraints, vendor execution or other lane was changed. This report is not authorization to combine the retained and checked candidates, and is not pushed pending parent review.
...
## First recommendation: reuse the real guard's registered completion pulse

For only the enabled registered controller, consider setting the existing forward_committed token from `guard_commit[0]` rather than re-evaluating `return_commit_valid && result_destination_ready && !next_inverse`. Leave the disabled branch literal and do not change guard or mailbox publication predicates.
...
Expected cost: zero added FF/RAM/DSP and one edge of private forward_committed latency. Healthy final bank publication, ACK, accepted arithmetic tuples and service cycles should remain unchanged because publication cannot occur before n+5. During the extra edge the ready-selection mux remains on kernel/product capacity rather than bank-valid: prove no harmful early ACK; reader ownership is unavailable before final publication. A later fault may coincide with consuming a previously valid pulse, but all unchanged current/sticky publication/ACK/quarantine fences remain; the retained token alone is not authority.

Do not reattach the wide current fault predicate to the new token D pin. The current fault must still veto the actual bank publication/ACK through their existing paths. The full fault cone still ends at the guard's existing commit_pulse/fault registers and public fences; this proposal removes the extra combinational propagation to forward_committed only. It does not fix the four ROM CE paths or guarantee timing closure.
...
Here the preferred first cut is only existing final-commit Q→retained forward token. The ROM representation change is a separate optional experiment. Neither is a wholesale checked-bank/read-queue union, a claim that read-side offered-summary fixes write-side faults, or a physical pass.
