...
cd $old_directory
puts [exec {PYTHON} $audit_helper --verify $evidence_dir]
...
"""
COPY_VERIFY = (
    f"puts [exec {PYTHON} $audit_helper --verify $evidence_dir --copied $source_dir]\n"
)
GENERIC_CHECK = """set physical_generics [get_property GENERIC [get_filesets sources_1]]
foreach {name value} [list REGISTERED_SCHEDULING $registered DISTRIBUTED_FAST_FAULT $distributed PRIVATE_NEXT_START_SCRATCH $scratch] {
...
def verify_prepared(prepared, copied=None):
    subprocess.run(
    ...
    if copied is not None:
        if {p.name for p in copied.glob("*.v")} != set(RTL):
            raise ValueError("copied runtime RTL closure differs")
...
def prepare(actual_dir, output):
    ...
    metadata = {
        "scope": "OFFLINE_ONLY_exact111_physical_preparation_NO_synthesis_route",
        "actual": actual,
...
def main():
    ...
    if args.verify is not None and args.actual is None and args.output is None:
        result = verify_prepared(args.verify, args.copied)
    elif (
        ...
        parser.error("choose --actual/--output or --verify with optional --copied")
