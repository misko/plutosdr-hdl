...
if {{$registered != 1 || $distributed != 1 || $scratch != 1 || $per_cause != 1 || $rom_word != 1 || $rom_metadata != 1}} {{ error "wrong combined physical options" }}
set prior_sources [file join $evidence_dir frozen_sources]
"""
...
foreach {name value} [list REGISTERED_SCHEDULING $registered DISTRIBUTED_FAST_FAULT $distributed PRIVATE_NEXT_START_SCRATCH $scratch PER_CAUSE_FAULT_CDC $per_cause PRIVATE_ROM_READ_AHEAD $rom_word PRIVATE_BLOCK_METADATA_READ_AHEAD $rom_metadata] {
...
def adapt_synthesis(original):
    changes = (
        ...
        (
            ...
            "  REGISTERED_SCHEDULING=$registered DISTRIBUTED_FAST_FAULT=$distributed \\\n  PRIVATE_NEXT_START_SCRATCH=$scratch PER_CAUSE_FAULT_CDC=$per_cause PRIVATE_ROM_READ_AHEAD=$rom_word PRIVATE_BLOCK_METADATA_READ_AHEAD=$rom_metadata] [get_filesets sources_1]\n"
            + GENERIC_CHECK,
        ),
        (
            ...
            (
                'puts $channel "registered_scheduling=$registered"\n'
                'puts $channel "distributed_fast_fault=$distributed; private_next_start_scratch=$scratch; per_cause_fault_cdc=$per_cause; private_rom_read_ahead=$rom_word; private_block_metadata_read_ahead=$rom_metadata"\n'
...
def verify_actual(actual):
    ...
    time = (actual.parent / "rom-actual-owner-v1/time.txt").read_text()
    if re.findall(r"^\s*Exit status: (\d+)$", time, re.MULTILINE) != ["0"]:
        raise ValueError("actual original process did not exit zero")
    launch = (actual.parent / "rom-actual-owner-v1/launch.log").read_text()
    if re.findall(
    ...
    owner = actual.parent / "rom-actual-owner-v1"
    ...
    if (actual.parent / "rom-actual-owner-v1/process-exit.txt").read_text() != "0\n":
    ...
    return {
        ...
        "launch_sha256": sha(actual.parent / "rom-actual-owner-v1/launch.log"),
        "time_sha256": sha(actual.parent / "rom-actual-owner-v1/time.txt"),
        "csv_sha256": hashes,
...
def prepare(actual_dir, output):
    ...
    metadata = {
        "scope": "OFFLINE_ONLY_exact111C1K1M1_physical_preparation_NO_synthesis_route",
        "actual": actual,
