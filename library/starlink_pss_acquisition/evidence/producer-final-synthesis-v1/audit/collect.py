...
copy(ROOT / 'product-final-route-v1/own_route.py', Path('route-proposal/own_route.py'))
copy(ROOT / 'rom-k1m1-route-v1/own_route.py', Path('route-proposal/own_route.original.py'))
baseline = ROOT / 'rom-k1m1-synth-v1/synthesis'
for name in ('utilization.rpt', 'hierarchy.rpt', 'resource_receipt.txt', 'cdc_unqualified.rpt', 'synthesis_clocks.rpt', 'scope.txt'):
    copy(baseline / name, Path('baseline-ROM-synthesis') / name)
