"""One-shot lossless packaging of terminal 10102; never run vendor tools."""
import gzip
import hashlib
import json
import runpy
...
result = api['verify_result'](SIM / 'simulate.log', 1, 1, PREPARED / 'frozen_sources')
...
save_json('fresh-read-only-audit.json', {
    'scope': 'actual_qualified_status_and_unconditional_ROM_NOT_raw217_or_physical',
    'original_handle': 10102, 'tool_observed_exit': 0, 'settings': settings,
...
for name in ('offline-verification.json', 'immediate-prelaunch-audit-v1.json',
             'authorized-invocation-v1.txt', 'independent-terminal-audit-v1.json'):
    copy_file(PREPARED.parent / name, 'independent/' + name)
copy_file(PREPARED.parent / 'tmp/prelaunch-write-probe-v1.txt',
          'independent/prelaunch-write-probe-v1.txt')
