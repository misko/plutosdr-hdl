...
module tb_starlink_pss_fft_bank_owned_slice;
 ...
 integer qualification_edges=0;
 reg qualification_clock;
 ...
 always @(posedge dut.return_commit_valid)begin
  qualification_edges=qualification_edges+1;
  qualification_clock=fft_clk;
 end
 task actual_added_fault_body;
    ...
    if (exact_extra_kind != 0) begin
      ...
      repeat (4) tick(); exact_extra_stalls = exact_extra_stalls + 1;
 ...
 initial begin
  ...
  if(qualification_edges!=1 || qualification_clock!==(PROVIDER==1))
   $fatal(1,"EDGE_DECLARED_PROVIDER_PHASE_NOT_REACHED");
  $display("EXTRA_EDGE_GUARD_PASS kind=%0d mode=%0d provider=%0d held_posedges=%0d fault_edges=%0d lead_fs=%0d exact_reasons=%h old_checks=%0d qualification_edges=%0d qualification_clock=%b synthetic_core_minimal_banks_not_fft=1",
   KIND,MODE,PROVIDER,held_edges,fault_edges,lead_fs,dut.result_guard.fault_reasons,old_checks,qualification_edges,qualification_clock);$finish;
 end
endmodule
