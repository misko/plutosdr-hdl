...
module tb_starlink_pss_fft_bank_owned_slice;
 parameter integer KIND=0,MODE=0;
 ...
 integer inverse_jobs=0,held_edges=0,fault_edges=0,old_checks=0;
 ...
 always @(posedge fft_clk)if(resetn)begin
  ...
  if(dut.event_last_missing)begin
   ...
   if(lead_ns<2.857142 || lead_ns>2.857144 || dut.return_commit_valid!==0 ||
 ...
 always @(posedge dut.event_last_missing)begin
  ...
  if(fft_clk!==0)$fatal(1,"EDGE_FAULT_DRIVE_NOT_NEGEDGE");
 ...
 initial begin
  ...
  $display("EXTRA_EDGE_GUARD_PASS kind=%0d mode=%0d held_posedges=%0d fault_edges=%0d lead_ns=%0.6f exact_reasons=%h old_checks=%0d synthetic_core_minimal_banks_not_fft=1",
   KIND,MODE,held_edges,fault_edges,lead_ns,dut.result_guard.fault_reasons,old_checks);$finish;
 end
endmodule
