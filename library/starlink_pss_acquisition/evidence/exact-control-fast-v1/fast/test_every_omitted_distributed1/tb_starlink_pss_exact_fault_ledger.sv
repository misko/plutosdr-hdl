...
module tb_starlink_pss_exact_fault_ledger;
  ...
  task automatic step;
    ...
    // Icarus evaluates procedural-force RHS once. Reapply every step and
    // assert the actual applied tuple, so no stale force can hide a cause.
    if ({dut.preparation_fault_now, dut.output_bank_fault, dut.result_fault,
         dut.handoff_fault_now, dut.product_bank_framing_fault_now, dut.product_bank_fault,
         dut.product_overflow, dut.kernel_fault, dut.vendor_fault_now, dut.source_fault_fast[1],
         dut.input_guard_fault, dut.input_fault_now} !== causes ||
        {old.preparation_fault_now, old.output_bank_fault, old.result_fault,
         old.handoff_fault_now, old.product_bank_framing_fault_now, old.product_bank_fault,
         old.product_overflow, old.kernel_fault, old.vendor_fault_now, old.source_fault_fast[1],
         old.input_guard_fault, old.input_fault_now} !== causes)
      $fatal(1, "EXACT_LEDGER_APPLIED_CAUSE_MISMATCH");
  ...
  initial begin
    ...
    for (mask = 0; mask < 4096; mask = mask + 1) begin
      ...
      causes = mask; step(); // all4096 current masks again with scalar already sticky
    end
