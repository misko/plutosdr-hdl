# Exact candidate runtime; requires separately approved actual result. Synthesis only; no route or timing waiver.
...
if {[file tail [info script]] ne "synthesize_retained_control.tcl"} { error "exact frozen runner filename required" }
...
set cli [file join $qualified source_snapshot tools prepare_starlink_retained_control_actual.py]
set qualified_sha 7a9b32f10241d22c3f5a6d3967ca9841e2646bb94d714f4935c05f5e8d03635e
...
set status [catch {
  ...
  foreach name {synthesize_retained_control.tcl clocks.xdc threads.tcl SHA256SUMS} {
  ...
  set generics [list KERNEL_ROM_FILE=$kernel ENABLE_RETAINED_OUTPUT=1 REGISTERED_SCHEDULING=1 BOUNDARY_ROUND_SAT=1 REGISTER_OPERANDS=1 LOCAL_FIRST_ADMISSION=1 PRIVATE_DESCRIPTOR_OFFER=1 CLOSED_INPUT_CUTOVER=1]
...
puts RETAINED_CONTROL_SYNTHESIS_RECORDED_NOT_TIMING_OR_DEPLOYMENT_PASS
