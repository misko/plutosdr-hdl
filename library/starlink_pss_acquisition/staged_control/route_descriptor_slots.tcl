...
foreach name {starlink_pss_descriptor_slots.v descriptor_slots_timing_probe.v route_descriptor_slots.tcl} {
  file copy [file join $source_dir $name] [file join $output $name]
}
read_verilog -sv [file join $output starlink_pss_descriptor_slots.v]
read_verilog -sv [file join $output descriptor_slots_timing_probe.v]
synth_design -top descriptor_slots_timing_probe -part xc7z010clg400-1 -mode out_of_context -flatten_hierarchy rebuilt
create_clock -period 5.714 -name island_175 [get_ports clk]
opt_design
place_design
phys_opt_design
route_design
write_checkpoint [file join $output descriptor_slots_routed.dcp]
...
puts DESCRIPTOR_SLOTS_ROUTE_RECORDED_NOT_RECEIVER_OR_BOARD_SIGNOFF
