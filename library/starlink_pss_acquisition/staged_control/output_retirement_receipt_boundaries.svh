  ...
  task automatic output_retirement_boundary(input integer boundary);
    ...
    begin
      ...
      if(boundary==0)begin
        ...
        release dut.output_replay_accept;aux_finish;
        ...
        else force dut.output_bank.input_metadata_certified=1'b0;
        #0.001;
        ...
        @(negedge fft_clk);
        release dut.core_status_valid;release dut.core_status_data;release dut.output_bank.input_metadata_certified;
        repeat(12)@(negedge fft_clk);
        repeat(100)begin
          @(negedge fft_clk);
  ...
  task automatic run_output_retirement_boundaries;
    begin
      for(out_retirement_boundary=0;out_retirement_boundary<5;out_retirement_boundary=out_retirement_boundary+1)
        output_retirement_boundary(out_retirement_boundary);
      $display("OUTPUT_RETIREMENT_BOUNDARIES_PASS cases=5 delayed_publication=1 pending_reset=1 checked_receipt=1");
    end
  endtask
