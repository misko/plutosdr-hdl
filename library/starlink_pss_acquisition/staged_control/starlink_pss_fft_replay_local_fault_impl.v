...
module starlink_pss_fft_replay_local_fault_impl #(
  ...
  assign product_bank_fault = product_ram_fault || product_stage_fault || forward_buffer_private_fault || replay_fifo_local_fault;
  ...
  wire result_destination_ready = next_inverse ? output_bank_ready :
    (forward_receipt_wait ? product_bank_valid : (forward_buffer_owned && !forward_buffer_private_fault));
  ...
  starlink_pss_replay_local_fault_ring #(.WIDTH(121)) replay_fifo (
    ...
    .output_data(replay_payload),.occupancy(replay_occupancy),.empty(replay_fifo_empty),.fault(replay_fifo_fault),.local_fault(replay_fifo_local_fault)
  );
  ...
  assign registered_quarantine = fast_fault || forward_buffer_private_fault || result_fault || (|epoch_input_reasons) ||
