...
.owner_fault_now(),
.owner_ack_accept());
starlink_pss_result_guard_commit_pulse #(.CERTIFIED_PRIVATE_ADMISSION(1),.PRIVATE_ACK_RETIREMENT(__PRIVATE_ACK__),.PRIVATE_QUARANTINE_OFFER(1),.USE_PRIVATE_DESCRIPTOR_OFFER(1),.ENABLE_OFFERED_FAULT_SUMMARY(1),.REQUIRE_KNOWN_COMPLETED_INPUT(1),.USE_PHASE_INPUT_FAULT(0),.USE_COMPLETED_INPUT_FAULT(1),.USE_PREFLIGHT_REASON_ONLY(1),.USE_FORWARD_RETIREMENT(1)) dut (
.clk(clk),
.resetn(resetn),
.job_valid(job_valid),
.private_descriptor_offer(private_descriptor_offer),
...
integer pulses=0,storage_edges=0,immediate_acks=0,unknown_pulses=0;
...
task compare;begin
 ...
 if(original.commit_pulse===1)begin
   pulses=pulses+1;
   if(dut.active_storage!==original.active_private || dut.ack_storage!==original.awaiting_ack)storage_edges=storage_edges+1;
   if(original.owner_ack_accept===1)immediate_acks=immediate_acks+1;
 end
 if(original.commit_pulse===1'bx)unknown_pulses=unknown_pulses+1;
...
task complete_job(input integer phase,input integer delay_ack,input integer mode);
 integer k;
 begin
  restart;inverse_phase=phase;job_descriptor={1'(phase),69'h1122334455667788};
  input_bank_reserved=1;output_bank_reserved=1;mailbox_input_ready=1;
  final_fence_certified=1;private_descriptor_offer=1;job_valid=1;tick;
  job_valid=0;private_descriptor_offer=0;
  for(k=0;k<512;k=k+1)begin
   certified_input_beat=1;offered_input_beat=1;
   certified_input_complete=k==511;offered_input_complete=k==511;
   core_event_frame_started=k==0;tick;
  end
  certified_input_beat=0;offered_input_beat=0;certified_input_complete=0;
  offered_input_complete=0;core_event_frame_started=0;completed_input_certified=1;
  if(mode==1 || mode==2)final_fence_certified=0;
  for(k=0;k<512;k=k+1)begin
   core_output_tvalid=1;core_output_tlast=k==511;
   core_output_tuser={3'b0,5'd7,7'b0,9'(k)};
   core_output_tdata=48'(k*1234567);
   core_status_tvalid=k==511;core_status_tdata=8'd7;tick;
  end
  core_output_tvalid=0;core_output_tlast=0;core_status_tvalid=0;
  if(mode==1)begin repeat(7)tick;final_fence_certified=1;end
  if(mode==2)begin final_fence_certified=1'bx;repeat(3)tick;final_fence_certified=1;end
  if(mode==3)begin external_fault_now=1;completed_input_fault_now=1;tick;repeat(3)tick;restart;end
  else begin
   tick;
   if(original.commit_pulse!==1 || original.owner_active!==0 || original.owner_awaiting_ack!==1)
    $fatal(1,"real completed frame did not establish pulse");
   if(mode==4)begin resetn=0;tick;restart;end
   else begin
    if(mode==5)mailbox_input_ready=1'bx;
    else if(delay_ack>0)mailbox_input_ready=0;
    if(mode==6)begin external_fault_now=1;completed_input_fault_now=1;end
    tick;
    repeat(delay_ack)tick;
    mailbox_input_ready=1;tick;tick;
    if(mode!=6 && (original.busy!==0 || original.protocol_fault!==0))
     $fatal(1,"pulse ACK/recovery did not finish");
    restart;
   end
  end
 end
...
initial begin
 restart;
 for(n=0;n<2;n=n+1)for(j=0;j<7;j=j+1)begin
  complete_job(n,0,j);complete_job(n,4,j);
 end
...
forward_mailbox_fault=four($unsigned($random(seed))%4);
 ...
 restart;force original.fault_reasons=8'h04;force dut.fault_reasons=8'h04;tick;
 release original.fault_reasons;release dut.fault_reasons;tick;
 restart;
 if(checks<50000)$fatal(1,"vacuous full guard comparison");
 if(pulses<10 || storage_edges<10 || immediate_acks<1 || unknown_pulses<1)$fatal(1,"vacuous pulse boundary coverage");
 $display("GUARD_PULSE_COMPONENT_PASS checks=%0d pulses=%0d storage_edges=%0d immediate_acks=%0d unknown_pulses=%0d four_state_causes=4096 random_cycles=20000",checks,pulses,storage_edges,immediate_acks,unknown_pulses);$finish;
end
endmodule
