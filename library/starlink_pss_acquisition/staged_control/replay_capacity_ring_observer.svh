  ...
  integer ring_private_writes=0,ring_offer_checks=0;
  reg ring_private_write_edge;
  always @(posedge fft_clk)begin
    ring_private_write_edge=dut.fast_running && dut.replay_fifo.push===1 && dut.forward_buffer_valid!==1;
    if(dut.fast_running)begin
      ring_offer_checks=ring_offer_checks+1;
      if(dut.forward_buffer_valid===1 && dut.forward_buffer_private_replay_valid!==1)
        $fatal(1,"public source lacks private offer");
      if(ring_private_write_edge)begin
        if(dut.forward_buffer_fault!==1 || dut.replay_fifo_fault!==1 ||
           dut.product_commit_authorized!==0 || dut.output_replay_accept!==0)
          $fatal(1,"private ring write without current cancellation");
        ring_private_writes=ring_private_writes+1;
      end
    end
    #0.001;
    if(ring_private_write_edge && dut.fast_running &&
       (dut.replay_fifo.fault_q!==1 || !dut.replay_fifo_empty || dut.replay_private_valid!==0))
      $fatal(1,"cancelled ring write retained ownership");
  end
  task automatic report_replay_ring;
    $display("REPLAY_RING_PASS checks=%0d private_writes=%0d publication_fenced=1",ring_offer_checks,ring_private_writes);
  endtask
