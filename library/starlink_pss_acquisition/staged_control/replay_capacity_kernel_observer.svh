  ...
  starlink_pss_kernel_rom #(.ROM_FILE("upper_edge_pss_kernel_q17.mem"),
    ...
    .input_ready(),.input_bin_index(dut.joiner.input_bin_index),
    .input_block_exponent(dut.joiner.input_block_exponent),.input_last(dut.joiner.input_last),
    .input_block_start_index(dut.joiner.input_block_start_index),
  ...
  task automatic compare_private_replay_kernel;
    begin
      ...
      if({dut.joiner.kernel_rom.expected_bin_index,dut.joiner.kernel_rom.expected_next_block_start,dut.joiner.kernel_rom.have_previous_block} !==
        ...
        if((dut.forward_buffer_fault!==1 && dut.replay_fifo_fault!==1) || dut.product_commit_authorized!==0 || dut.output_replay_accept!==0 ||
  ...
  always @(posedge fft_clk)begin
    ...
    if(replay_was_running)begin
      ...
      if(replay_only_this_edge)begin
        ...
        if(dut.joiner.input_last)private_replay_final_only=private_replay_final_only+1;
      end
    end
    #0.001;
