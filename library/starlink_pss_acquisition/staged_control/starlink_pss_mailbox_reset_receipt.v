...
module starlink_pss_mailbox_reset_receipt #(
  ...
  // Reset receipts only observe local reset application and settled state.
  // The first synchronizer taps must feed ONLY their second stages. While
  // !running, a local edge resets both stages and sets this receipt together.
  // The epoch barrier still requires two idle observations before release.
  reg writer_reset_applied, reader_reset_applied;
  always @(posedge input_clk) writer_reset_applied <= !in_running;
  always @(posedge output_clk) reader_reset_applied <= !out_running;
  // BEGIN RETAINED MAILBOX OBSERVATIONS: local reset qualification.
  assign owner_request = request_toggle;
  assign owner_ack_sync = acknowledge_sync[1];
  assign writer_reset_idle = !in_running && writer_reset_applied === 1'b1 &&
    request_toggle === 1'b0 && acknowledge_sync[1] === 1'b0 &&
    write_position === {ADDRESS_WIDTH{1'b0}} && input_fault === 1'b0;
  assign reader_reset_idle = !out_running && reader_reset_applied === 1'b1 &&
