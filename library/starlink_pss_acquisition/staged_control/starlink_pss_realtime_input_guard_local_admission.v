...
module starlink_pss_realtime_input_guard_local_admission #(
  ...
  // Opt-in caller wires input_metadata to the original phase-selected bus.
  ...
  initial begin
    if (BANK_LOCAL_IDENTITY !== 0 && BANK_LOCAL_IDENTITY !== 1)
  ...
  // BEGIN BANK LOCAL IDENTITY
  // Compare the two held bank buses before the scheduler selects one bit.
  // There is no registered/delayed validity and all 70 bits remain checked.
  // X/Z selector: preserve equality of the original bitwise-merged bus.
  wire identity_matches;
  generate if (BANK_LOCAL_IDENTITY) begin : bank_local_identity
    wire [1:0] bank_matches;
    for (genvar bank = 0; bank < 2; bank = bank + 1) begin : banks
      wire [69:0] bank_metadata = bank == 0 ? source_bank_metadata : product_bank_metadata;
      (* keep = "true" *) wire [23:0] leaf_equal;
      (* keep = "true" *) wire [3:0] group_equal;
      for (genvar leaf = 0; leaf < 24; leaf = leaf + 1) begin : leaves
        localparam integer BITS = leaf == 23 ? 1 : 3;
      ...
      assign bank_matches[bank] = &group_equal;
    end
    wire known_phase = bank_phase === 1'b0 || bank_phase === 1'b1;
    assign identity_matches = known_phase ?
      (bank_phase ? bank_matches[1] : bank_matches[0]) : selected_identity_matches;
  end else begin : original_bank_identity
    assign identity_matches = selected_identity_matches;
  end endgenerate
  // END BANK LOCAL IDENTITY
