# Vivado 2022.2 OOC gate for exact exponent-aware score-ratio preparation.
# Usage: vivado -mode batch -source synthesize_score_prepare_ooc.tcl \
#        -tclargs OUTPUT
...
read_verilog [file join $script_dir starlink_pss_score_prepare.v]
read_xdc [file join $script_dir starlink_pss_score_prepare_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_score_prepare \
...
foreach {name contents} [list \
    starlink_pss_score_prepare_utilization_opt.rpt $utilization_report \
    starlink_pss_score_prepare_timing_opt.rpt $timing_report] {
...
set methodology_file [open \
  [file join $output_dir starlink_pss_score_prepare_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_score_prepare_check_timing.rpt] w]
...
if {[llength $ramb36_cells] != 0 || [llength $ramb18_cells] != 0} {
  error "score preparation must not consume BRAM"
}
if {[llength $dsp_cells] != 4} {
  error "expected four score-preparation DSPs, got [llength $dsp_cells]"
}
...
foreach {label pattern} {
  total_luts {\| starlink_pss_score_prepare +\| +\(top\) +\| +([0-9]+) +\|}
  total_ffs {\| starlink_pss_score_prepare +\| +\(top\) +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +[0-9]+ +\| +([0-9]+) +\|}
} {
...
if {$total_luts > 2000 || $total_ffs > 1200} {
  error "score-preparation logic budget exceeded: LUT=$total_luts FF=$total_ffs"
}

write_checkpoint -force \
  [file join $output_dir starlink_pss_score_prepare_opt.dcp]
set summary [open \
  [file join $output_dir starlink_pss_score_prepare_ooc_summary.txt] w]
puts $summary "vivado_version=[version -short]"
puts $summary "part=xc7z010clg400-1"
puts $summary "correlation_component_bits=24"
puts $summary "sample_energy_bits=38"
puts $summary "coefficient_energy=1073742825"
puts $summary "ratio_bits=69"
puts $summary "pipeline_stages=3"
...
puts "STARLINK_SCORE_PREPARE_OOC_PASS setup_wns_ns=$setup_wns hold_whs_ns=$hold_whs total_luts=$total_luts total_ffs=$total_ffs dsp48e1=[llength $dsp_cells]"
close_design
