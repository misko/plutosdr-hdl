...
output_dir="${1:-${script_dir}/build/xfft-block-adapter-ooc}"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$script_dir/synthesize_xfft_block_adapter_ooc.tcl" \
      -tclargs "$output_dir" 2>&1 | tee xfft_block_adapter_ooc_transcript.log
)
