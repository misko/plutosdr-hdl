# Vivado 2022.2 OOC gate for the atomic result-publication milestone.
# Usage: vivado -mode batch -source synthesize_result_store_ooc.tcl -tclargs OUTPUT
...
read_xdc [file join $script_dir starlink_pss_result_store_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -directive AreaOptimized_high \
  -top starlink_pss_result_store \
  -part xc7z010clg400-1
opt_design -directive ExploreArea

write_checkpoint -force \
  [file join $output_dir starlink_pss_result_store_opt.dcp]
...
foreach {name contents} [list \
    starlink_pss_result_store_utilization_opt.rpt $utilization_report \
    starlink_pss_result_store_timing_opt.rpt $timing_report] {
...
set cdc_file [open \
  [file join $output_dir starlink_pss_result_store_cdc_opt.rpt] w]
puts -nonewline $cdc_file $cdc_report
close $cdc_file

set methodology_report [report_methodology -return_string]
set methodology_file [open \
  [file join $output_dir starlink_pss_result_store_methodology_opt.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_result_store_check_timing.rpt] w]
...
foreach category $expected_check_timing_categories {
  ...
  if {$category eq "no_input_delay" && $category_count == 2 &&
      [regexp {There are 2 input ports with no input delay but user has a false path constraint} \
        $check_timing_report]} {
    continue
  }
  if {$category_count != 0} {
...
regsub -all {checking no_input_delay \(2\)} $check_timing_unexpected \
...
if {[llength $dsp_cells] != 0} {
  error "result store must infer zero DSP48E1 cells, got [llength $dsp_cells]"
}
set ram_cells [get_cells -quiet -hier -filter {REF_NAME =~ RAMB18*}]
if {[llength $ram_cells] != 1} {
  error "result store must infer exactly one RAMB18 cell, got [llength $ram_cells]"
}
...
if {$slice_luts > 400} {
  error "result-store LUT budget exceeded: $slice_luts > 400"
}
if {$slice_registers > 250} {
  error "result-store register budget exceeded: $slice_registers > 250"
}
if {$block_ram_tiles != 0.5} {
  error "result store must use one RAMB18 (0.5 tile), got $block_ram_tiles"
}
if {$utilization_dsps != 0} {
  error "result-store utilization reports nonzero DSPs: $utilization_dsps"
}

set summary_path [file join $output_dir starlink_pss_result_store_ooc_summary.txt]
...
puts $summary "check_timing_expected_false_pathed_reset_no_input_delay=2"
...
puts $summary "ramb18e1=[llength $ram_cells]"
puts $summary "dsp48e1=[llength $dsp_cells]"
close $summary

puts "STARLINK_RESULT_STORE_OOC_PASS setup_wns_ns=$setup_wns slice_luts=$slice_luts slice_registers=$slice_registers block_ram_tiles=$block_ram_tiles ramb18e1=[llength $ram_cells] dsp48e1=[llength $dsp_cells]"
close_design
