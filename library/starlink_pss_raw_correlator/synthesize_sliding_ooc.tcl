# Vivado 2022.2 OOC gate for the cached-Eh/sliding-Ex correlator milestone.
# Usage: vivado -mode batch -source synthesize_sliding_ooc.tcl -tclargs OUTPUT
...
read_xdc [file join $script_dir starlink_pss_sliding_correlator_ooc.xdc]

synth_design \
  -mode out_of_context \
  -flatten_hierarchy rebuilt \
  -top starlink_pss_sliding_correlator \
  -part xc7z010clg400-1

write_checkpoint -force \
  [file join $output_dir starlink_pss_sliding_correlator_synth.dcp]
...
set utilization_file [open \
  [file join $output_dir starlink_pss_sliding_correlator_utilization_synth.rpt] w]
puts -nonewline $utilization_file $utilization_report
close $utilization_file
set timing_file [open \
  [file join $output_dir starlink_pss_sliding_correlator_timing_synth.rpt] w]
puts -nonewline $timing_file $timing_report
close $timing_file

set methodology_report [report_methodology -return_string]
set methodology_file [open \
  [file join $output_dir starlink_pss_sliding_correlator_methodology_synth.rpt] w]
...
set check_timing_file [open \
  [file join $output_dir starlink_pss_sliding_correlator_check_timing.rpt] w]
...
if {$slice_luts > 2500 || $slice_registers > 2000 || $block_ram_tiles > 5.0} {
  error "milestone budget exceeded: LUT=$slice_luts FF=$slice_registers BRAM=$block_ram_tiles"
}

set summary_path [file join $output_dir starlink_pss_sliding_correlator_ooc_summary.txt]
...
puts "STARLINK_SLIDING_OOC_PASS setup_wns_ns=$setup_wns slice_luts=$slice_luts slice_registers=$slice_registers block_ram_tiles=$block_ram_tiles dsp48e1=[llength $dsp_cells]"
close_design
