# Full MAC + normalization arithmetic, registered-boundary probe only.
set origin [file dirname [file normalize [info script]]]
if {$argc != 1} { error "usage: route_datapath.tcl ABSOLUTE_ARTIFACT_DIRECTORY" }
set output [file normalize [lindex $argv 0]]
if {[file exists $output]} { error "refusing to overwrite route evidence" }
file mkdir $output
cd $origin
set_param general.maxThreads 2
foreach source {starlink_coarse25_mac.v starlink_coarse25_score.v starlink_coarse25_datapath.v starlink_coarse25_datapath_probe.v} {
  read_verilog [file join $origin $source]
}
read_verilog [file join $origin .. starlink_pss_acquisition starlink_pss_score_divider.v]
synth_design -top starlink_coarse25_datapath_probe -part xc7z010clg400-1 -mode out_of_context
...
puts "COARSE25_DATAPATH_PROBE_ONLY setup_slack=$wns hold_slack=$whs full_board_qualified=0"
if {$wns < 0 || $whs < 0} { error "registered-boundary datapath timing failed" }
